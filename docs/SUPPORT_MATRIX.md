...
| PFD/PLN/PNT geometry           | yes                     | partial          | coordinate bounds               | Leaflet                 | no; GeoJSON helper is library-only | yes                                | partial |
| GRD Type 2 compact profile     | yes                     | yes              | dimensions, layout and bytes    | viewport-cull canvas    | CSV, map PNG, variant ZIP          | yes                                | yes     |
| GRD Type 1                     | raw                     | no               | metadata/unsupported status     | no                      | raw only                           | yes                                | no      |
| TZN/PDV ordering               | yes                     | yes              | layout/reference evidence       | independent layers      | CSV/variant ZIP                    | yes                                | yes     |
| Multiple PDVs per Type 2 cell  | yes                     | yes              | yes                             | independent layers      | CSV/variant ZIP                    | yes                                | yes     |
...
| Time-log Type 1 XML            | yes                     | PTN/DLV/DET/DDI  | counterpart/layout/references   | executed tree/inspector | channel CSV, map PNG               | yes                                | yes     |
| Time-log Type 1 BIN            | yes                     | sparse records   | indices/truncation/resource cap | point canvas/table      | channel CSV, map PNG               | yes                                | yes     |
...
| Guarded package variants       | yes                     | supported subset | preflight plus normal re-import | selected after creation | ZIP                                | source dataset retained separately | yes     |
