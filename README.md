...
- Frame-coalesced Leaflet canvas rendering with cached planned-grid rasters, viewport/density-aware executed points, complete value cards on cell/point hover, persistent click selection, field boundary, described map controls, and streets/satellite backgrounds retained through the full supported zoom range.
...
`pnpm fixtures:performance` generates an ignored browser-test package under `output/playwright` with 40,000 planned cells and 50,000 executed points. It is the reproducible interaction benchmark for large map layers.
