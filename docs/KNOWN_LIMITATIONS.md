...
- Time-log XML/BIN files are indexed and preserved; record decoding and playback are not implemented.
...
- Planned-versus-executed analysis and time animation remain future adapters. Active planned layers show a finite-value average and an area-weighted total only for recognized per-hectare or per-square-metre units, using the declared constant cell area and current display filters.
...
- Active-channel CSV and map PNG are the current user-facing data exports. GeoJSON serialization exists as a library helper but is not yet wired to a control; validation and object-registry JSON exports are not implemented.
