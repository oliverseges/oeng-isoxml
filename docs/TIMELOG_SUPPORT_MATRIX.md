...
| Executed map display filters        | zero values, field clipping, outliers   |
...
Executed channel rows show both the hexadecimal DDI and its dictionary name. Map controls expose the same descriptions on pointer hover and keyboard focus. Planned and executed maps share one display-filter menu and the same persisted settings for hiding zero/empty values, cutting data to the task field boundary, and hiding conservative 3× IQR outliers. Executed maps apply those settings to point rendering, color classification, hit-testing, selection, visible-point totals, and exported screenshots. Field clipping is disabled when the viewer cannot resolve a boundary for the active task.

Planned and executed maps both expose the same screen-capture control. The exported PNG composes the visible background tiles, spatial boundary, active data layer, compact legend and required basemap attribution. Planned legends retain only calculated average and total dose rows; executed legends omit outlier and source diagnostics. A persistent selected cell or record is exported with its outline and value card when its anchor is inside the current viewport, while transient hover and coordinate-pin overlays remain omitted.
...
Clicking a decoded time-log row opens or closes its DDI channel branch and activates the log's first channel when switching logs. Collapsing the already active log changes only the tree, so it does not reopen or flash the Records panel. Grid rows follow the same combined navigation behavior; global Expand and Collapse remain available for the complete tree.
